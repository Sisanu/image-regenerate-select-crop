function sirsc_send_ajax_post_element(url, callback_action, elem, target) {
    ...
    var poz = jQuery('#' + target).position();
    var cont = jQuery('#' + target).html();
    var contPost = data;
    if (jQuery('#' + target).size()) {
        jQuery('#' + target).append('<div class="spinner" style="top:' + poz.top + 'px; left:' + poz.left + 'px;  height:' + jQuery('#' + target).innerHeight() + 'px; width:' + jQuery('#' + target).innerWidth() + 'px;"></div>');
    }

    var post_data = {
        action: url, // This is required so WordPress knows which func to use
        'sirsc_data': contPost // Post any variables you want here
    };
    jQuery.post(
        ajaxurl,
        post_data,
